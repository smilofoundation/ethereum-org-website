...
__To support private transactions in Smilo, the `web3.eth.sendTransaction(object)` API method has been modified.__
...
__To support sending raw transactions in Smilo, the `web3.eth.sendRawPrivateTransaction(string, object)` API method has been created.__
...
__Important:__ Please note that before calling this API, a `storeraw` api need to be called first to Smilo's private transaction manager. Instructions on how to do this can be found [here](../../Privacy/Tessera/Usage/Interface%20&%20API/).
...
```js
 ...
 var privateKey = new Buffer('e331b6d69882b4cb4ea581d88e0b604039a3de5967688d3dcffdd2270c0fd109', 'hex')
  var rawTx = {
   ...
   value: '0x00', 
   // This data should be the hex value of the hash returned by Smilo's privacy transaction manager after invoking storeraw api
...
```
#### eth.fillTransaction
To support offline signing of transaction. This api fills and defaults `RLP` plus `json`. This can be used to fill and sign both public and private transactions
...
##### Returns
- `raw`: `RLP` encoded bytes for the passed transaction object
...
__In addition to the JSON-RPC provided by Ethereum, Smilo exposes below two API calls.__
