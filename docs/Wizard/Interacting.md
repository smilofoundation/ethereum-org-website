...
After following the instructions in [Getting Started](./GettingStarted.md), you should have a fully generated local Smilo network. Here are some ways you can interact with the network to try out the features of Smilo.
...
Note: Run `./stop.sh` if you want to stop all Smilo/geth, tessera, and cakeshop instances running on your machine

## Demonstrating Privacy
The network comes with some simple contracts to demonstrate the privacy features of Smilo.  In this demo we will:
...
### Inspecting the Smilo nodes

We can inspect any of the Smilo nodes by using `./attach.sh` to open the Geth JavaScript console.  For this demo, we will be inspecting Node 1, Node 2, and Node 3.  
...
You can also try to do all of the above steps using the Storage contract in [Remix](http://remix.ethereum.org) by using our [Smilo Plugin](../RemixPlugin/Overview.md). Follow the instructions for activating the remix plugin in [Getting Started](../RemixPlugin/GettingStarted.md), connect to the nodes using their Smilo and Tessera urls:

Node 1 defaults:

- Smilo RPC: `http://localhost:22000`
- Tessera: `http://localhost:9081`

Node 2 defaults:

- Smilo RPC: `http://localhost:22001`
- Tessera: `http://localhost:9082`

Node 3 defaults:

- Smilo RPC: `http://localhost:22002`
