...
<tr>
    ...
    <td>Unable to read the secret key (password) from file specified by <code>Blackbox_CONFIG_SECRET</code><br>
...
<tr>
    ...
    <td>Secret key (password) could not be read from console or password file (see <code>Blackbox_CONFIG_SECRET in docs</code>).<br>
...
The level of logging is controlled by the logback configuration file. The default file packaged with Blackbox can be seen [here](https://github.com/smilofoundation/Blackbox/blob/master/Blackbox-dist/Blackbox-launcher/src/main/resources/logback.xml).
