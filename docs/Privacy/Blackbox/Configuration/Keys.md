!!! warning "Change from Blackbox v0.10.2+"
    The `keys.keyData.passwords` field is no longer supported as of Blackbox v0.10.2.  
...
Blackbox uses cryptographic keys to provide transaction privacy.  

You can use existing private/public key pairs as well as use Blackbox to generate new key pairs for you.  See [Generating & securing keys](../../Blackbox%20Services/Keys/Keys) for more info.
...
Passwords must be provided so that Blackbox can decrypt and use the private keys.  Passwords can be provided in multiple ways:
...
| CLI    | Blackbox will prompt on the CLI for the passwords of any encrypted keys that have not had passwords provided in the config.  This process only needs to be performed once, when starting the node.              |
...
!!! info
    Environment variables must be set if using an Azure Key Vault, for more information see [Setting up an Azure Key Vault](../../Blackbox%20Services/Keys/Setting%20up%20an%20Azure%20Key%20Vault)
...
Blackbox requires TLS certificates and keys to be stored in `.jks` Java keystore format.  If the `.jks` files are password protected then the following environment variables must be set: 

* `HASHICORP_CLIENT_KEYSTORE_PWD`
* `HASHICORP_CLIENT_TRUSTSTORE_PWD` 

!!! info
    If using a Hashicorp Vault additional environment variables must be set and a version 2 K/V secret engine must be enabled.  For more information see [Setting up a Hashicorp Vault](../../Blackbox%20Services/Keys/Setting%20up%20a%20Hashicorp%20Vault).
...
!!! info
    Environment variables must be set if using an _AWS Secrets Manager_, for more information see [Setting up an AWS Secrets Manager](../../Blackbox%20Services/Keys/Setting%20up%20an%20AWS%20Secrets%20Manager)

## Providing key passwords at runtime
Blackbox will start a CLI password prompt if it has incomplete password data for its locked keys.  This prompt can be used to provide the required passwords for each key without having to provide them in the configfile itself.  

For example:   

```bash
Blackbox -configfile path/to/config.json
...
# Blackbox startup continues as normal
``` 

## Multiple Keys
If wished, multiple key pairs can be specified for a Blackbox node. In this case, any one of the public keys can be used to address a private transaction to that node. Blackbox will sequentially try each key to find one that can decrypt the payload. This can be used, for example, to simplify key rotation.
...
For Blackbox v0.10.2 onwards the ThirdParty API `/keys` endpoint can be used to view the public keys of the key pairs currently in use by your Blackbox node. 

For Blackbox v0.10.1 and earlier, an ADMIN API endpoint `/config/keypairs` exists to allow you to view the public keys of the key pairs currently in use by your Blackbox node.  
