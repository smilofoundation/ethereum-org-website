...
## What does the Blackbox Enclave do?

The Blackbox Enclave is designed to handle all of the encryption/decryption operations required by the Transaction Manager, as well as all forms of key management.
...
The Blackbox Enclave **handles** the following data:
...
If using the individual "make-your-own" jars, you will need the "core Transaction Manager" jar along with the "Enclave clients" jar, and add them both to the classpath as such: `java -cp /path/to/transactionmanager.jar:/path/to/enclave-client.jar com.quroum.Blackbox.Launcher -configfile /path/to/config.json`
